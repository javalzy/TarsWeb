...
if [ "${SLAVE}" != "true" ]; then
  ...
  if [ ! -d ${TARS_INSTALL}/web ]; then
      ...
      echo "cd ${TARS_INSTALL}; git clone https://github.com/javalzy/TarsWeb.git web"
      exit 1
  fi
...
# yarn config set registry ${MIRROR}/npm/; yarn global add npm pm2
yarn config set registry ${MIRROR}/npm/; yarn global add pm2
